...
app.get("/reminders/completed", (c) => {
  ...
  return reminders.length ? c.json(reminders) : c.json({ error: "No completed reminders found" }, 404);
});

app.get("/reminders/not-completed", (c) => {
  const reminders = db.getRemindersByCompletion(false);
  return reminders.length ? c.json(reminders) : c.json({ error: "No pending reminders found" }, 404);
});

app.get("/reminders/due-today", (c) => {
  const reminders = db.getRemindersDueToday();
  return reminders.length ? c.json(reminders) : c.json({ error: "No reminders due today" }, 404);
});
